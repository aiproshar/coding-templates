...
Initially each set is root of its own. For Any DSU operation, we need to implement two methods
...
```python
# Initially all nodes are their own leader
...
find figures out the root/leader of any node. How we do it ? remember, a root is that node where it itself is its parent. So we continue climbing up until we find the parent
...
# Then iterate/traverse up and update the values
...
The union function returns true if both nodes were in different union and they are merged, or false if they are already in the same set

The union function returning false means they have detected a cycle

How can we be sure ? If both nodes leader is same, then they are in the same set
